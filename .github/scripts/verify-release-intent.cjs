...
/** @version 4 — verify-release-intent script version. Bump when behavior changes. */
const VERIFY_RELEASE_INTENT_SCRIPT_VERSION = 4;
...
function main() {
  ...
  if (!config) {
    fail(
      'No version config found (.sdlc-v2/config.toml ".version" section). ' +
