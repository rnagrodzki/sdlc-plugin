...
Present when `plan.guardrails` are configured in `.sdlc-v2/config.json`. Produced by plan Step 4.
...
`plan` reads a **template** to decide which section headers a plan must carry, and what each
