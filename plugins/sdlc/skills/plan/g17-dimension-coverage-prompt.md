...
Use this template in plan Step 3 when dispatching the G17 Dimension Coverage subagent.
...
Task tool (general-purpose):
  ...
  prompt: |
    ...
    findings that plan Step 4 splices into the plan as a
    ...
    **Pure-refactor suppression**: Do NOT fire B-criteria when the Description explicitly indicates rename-only, formatting, type-narrowing without semantic change, or dead-code removal. Look for markers like "rename-only", "no behavior change", "pure refactor", "formatting only".
    ...
    ### Step I — Defer check against harden learnings

    Read the last 100 lines of `{LEARNINGS_LOG_PATH}` (if it exists; skip if absent).
    Parse `## YYYY-MM-DD — harden:` headers within `{PR_COMMIT_WINDOW}`.
    ...
    harden entry's `Dimensions:` list. This prevents duplicate proposals
    within an active PR window where harden already proposed the same dimension.
    ...
    X1 action line MUST reference `setup/setup-dimensions.md` Step 8 transform
    by path — do NOT inline the transform. Example action:
    `regenerate .github/instructions/<name>.instructions.md per setup/setup-dimensions.md Step 8 transform`
    ...
    - Execute the Copilot transform inline — reference `setup/setup-dimensions.md` Step 8 by path only.
