...
| `mcp-output-drives-behavior` | mcp | warning | Every tool result must include a `next` hint naming the exact next action per outcome; a genuinely terminal tool may omit it if its description says so. A result whose next step is implied but unstated leaves the LLM to guess. |
