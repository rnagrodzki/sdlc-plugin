...
| `mcp-output-drives-behavior` | mcp | warning | Every tool result should tell the LLM what to do next, via a root-level `next` field the renderer hoists to the result's Next line; a genuinely terminal tool may omit it if its description says so. A result whose next step is implied but unstated leaves the LLM to guess. |
