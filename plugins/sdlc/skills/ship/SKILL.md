...
**6b. Release-level confirmation.** Only when `version` is present in `flags.steps` — a run with no `version` step has no release intent to confirm, skip silently. Read `flags.bump` and `sources.bump` from `ship_prepare` (step 4) — the already-resolved bump level (cli > config > built-in default). Under `flags.auto`, skip the prompt entirely: no interactive confirmation happens in auto mode, so the level is always `releaseSource:"config"` (`pr_apply` rejects `releaseSource:"user"` under `autoMode` regardless of what's passed here). Otherwise, AskUserQuestion:
...
On option 2: ask which level (`major`/`minor`/`patch`, optionally with an RC pre-release label) and use it in place of `flags.bump` for the rest of this run. Hold the resolved level and its `releaseSource` in working context: pass the level as the version step's `<bump>` dispatch arg (below), and forward `releaseSource` to the pr step's dispatch alongside `releaseLevel`/`releaseNotes`/`releasePreRelease` (see Decisions & gates → Capture release intent).
...
### pr
Tracking: `action:"begin-step"` → `action:"complete-step"`. Dispatch: Agent → pr, model sonnet, args `[--draft] [--base <branch>] [--auto]` (`--auto` = `flags.auto` — required so the pr skill's Step 1b release-intent gate and `pr_apply`'s `autoMode` enforcement both see it; omitting it would leave the pr skill unaware it's running inside an auto ship). When the version step ran, in addition to its own dispatch contract, instruct the Agent to include `releaseLevel`, `releaseNotes`, `releaseSource` (held from step 6b — `"config"` or `"user"`), and (when set) `releasePreRelease` — the values captured from the version step's artifacts plus step 6b's confirmed source — in its own `pr_apply` call. When `version` is absent from `flags.steps` (deliberately excluded from this run — step 6b never ran, there is no release intent to forward), instead instruct the Agent to pass `skipReleaseCheck: true` to its `pr_apply` call — otherwise `pr_apply` rejects the empty `releaseLevel` as an unacknowledged omission. No pause.
