...
**10b (execute-drift drafts).** Call `execute_state({action:"read"})` → `pendingIssueDrafts` (array of `{title, body, labels?, taskId?, timestamp?}`, accumulated by the execute step's `issue-draft` action — see Task 10). This part of 10b runs unconditionally, **including under `--auto`** — GH issue creation always requires approval per the auto-mode contract, unlike the rest of step 10b above which `--auto` skips. If `pendingIssueDrafts` is absent or empty, skip silently: no output, no prompt. Otherwise:
- Render the drafts as a numbered list, one line per draft: `N. {title} — {first line of body}`.
- Ask one batch AskUserQuestion presenting every draft as a selectable option, so the user can approve any subset in a single round-trip:
  > {N} issue draft(s) surfaced during this run's execute step. Approve the ones worth tracking.
