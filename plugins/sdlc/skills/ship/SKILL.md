...
### execute
Tracking: `action:"begin-step"` → `action:"complete-step"`. Dispatch: Agent → execute, model sonnet, args `--quality {flags.quality} --wave-timeout {flags.executeWaveTimeout} --wave-interval {flags.executeWaveInterval} <plan-path>` (never `--branch`, never `--auto`). Pauses only if plan-mode-blocked.
