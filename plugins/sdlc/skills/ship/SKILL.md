...
**Critical constraints** (full detail: [`reference.md`](reference.md)): no worktree isolation anywhere in this pipeline (`execute` uses a plain feature branch, set up before dispatch); call `ship_prepare` **at most once** per run — it initializes state as a side effect, and a second call re-seeds `steps[]` and destroys progress; this pipeline never creates or pushes a git tag itself — release intent (a bump level, optional pre-release label, and drafted notes) is resolved from `flags.bump` and forwarded to the pr step, which calls `pr_apply` to attach a `release:<level>` label and inject markers into the PR body, and the actual version bump/tag/CHANGELOG write happens post-merge via CI; a feature-branch push is auto-approved when `automation.push.featureBranchAutoApprove` is true (config, or forced true under `automation.mode: "unattended"` — known limitation: an explicit `false` is indistinguishable from absent due to the bool zero value, so unattended mode forces it true regardless) — otherwise the `pr` step still pauses for a manual `AskUserQuestion`; a default-branch push is never auto-approved regardless of config — `ship_prepare` returns a `DomainError` outright whenever the resolved branch is `main`/`master` and `pr` is in the step list; the branch recorded at `execute`'s init is enforced server-side — a mid-session checkout to a different branch returns a `DomainError`, not a silent follow; `ship_state{action:"cleanup"/"cleanup-pipeline"}` **stamps** the state file terminal (`pipelineStatus:"completed"`) — it does not delete it.
