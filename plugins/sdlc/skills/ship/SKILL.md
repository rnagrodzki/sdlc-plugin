...
**Review-findings ledger.** When the review step ran, close the summary with one ledger that accounts for **every** finding: the total parsed by the review routing (Decisions & gates), the count `received-review` reported fixed, and the deferred entries grouped by their own `reason` (each `deferredFindings` entry from the final `read` carries one):
...
**10b (execute-drift drafts).** This part is **triage only** — it does not persist anything. Every draft is already durable before this step runs: `execute_state({action:"issue-draft"})` writes it to `.sdlc-v2/history/deferred.json` at creation, with `source:"execute-drift"` and id `execute-drift-<timestamp>-<N>` (that draft's own RFC3339 `timestamp`, plus its 1-based position). So there is nothing here to promote, and **no `deferred_add` call is needed or wanted** — making one would only duplicate an entry that exists. Drafts survive an early exit from this pipeline, and survive state-file GC.

Call `execute_state({action:"read"})` → `pendingIssueDrafts` (array of `{title, body, labels?, taskId?, timestamp?}`, accumulated by the execute step's `issue-draft` action). This part of 10b runs unconditionally, **including under `--auto`** — GH issue creation always requires approval per the auto-mode contract, unlike the rest of step 10b above which `--auto` skips. If `pendingIssueDrafts` is absent or empty, skip silently: no output, no prompt. Otherwise:
- Render the drafts as a numbered list, one line per draft: `N. {title} — {first line of body}`.
- Ask one batch AskUserQuestion presenting every draft as a selectable option, so the user can mark any subset in a single round-trip:
  > {N} issue draft(s) surfaced during this run's execute step. All are already recorded for follow-up. Mark the ones worth filing as GitHub issues now.
- Unmarked drafts need no action: they stay open in `deferred.json` for a later run or for `/sdlc:deferred`.
- Filing a marked draft as a GitHub issue follows the filing steps of the same triage flow in [the deferred skill's `reference.md`](../deferred/reference.md), not a second copy. Use the draft's own `title` and `body` as the issue text, and pass its id (`execute-drift-<timestamp>-<N>`, as above) to `ship_state({action:"deferred_resolve", detail:{id:<id>}})` once the issue exists. Deferring that to a later run is always acceptable — nothing is lost by skipping it, so this step never blocks the rest of the pipeline.
