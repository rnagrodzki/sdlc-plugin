...
### execute
Tracking: `action:"begin-step"` → `action:"complete-step"`. Dispatch: Agent → execute, model sonnet, args `{flags.executeDispatchArgs} <plan-path>` (never `--branch`, never `--auto`) — `executeDispatchArgs` is computed by `ship_prepare` and only includes `--quality` when quality was actually set. Pauses only if plan-mode-blocked.
