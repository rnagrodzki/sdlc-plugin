...
### version
Tracking: `action:"begin-step"` → `action:"complete-step"`. Dispatch: Agent → version, model haiku, args `[<bump>] [--pre <label>] [--auto]`. Pauses YES — manual tag-and-push (see Decisions & gates).
...
### pr
Tracking: `action:"begin-step"` → `action:"complete-step"`. Dispatch: Agent → pr, model sonnet, args `[--draft] [--base <branch>]`. No pause.
...
- **Manual tag-and-push pause (after version's complete-step).** `AskUserQuestion` — even under `flags.auto` — asking the user to `git tag <NEW_TAG> && git push origin <NEW_TAG>` (version's own dispatch must report `NEW_TAG` in its artifacts). Then `ship_verify_side_effect({step:"version", expected:NEW_TAG}) → {landed}`; re-prompt if `landed:false`. Then `verify_tag_ancestry({tag:NEW_TAG}) → {ok, details}` — `ok:false` stops the pipeline before `pr`, showing `details`. If `version` isn't configured, this whole gate is skipped (nothing to check).
