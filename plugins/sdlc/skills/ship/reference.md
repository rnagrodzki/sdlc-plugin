...
**Feature-branch push is config-gated (KD-1); default-branch push is a hard gate, not config-gated.** `automation.push.featureBranchAutoApprove` (explicit config, or forced `true` under `automation.mode: "unattended"` — known limitation: an explicit `false` is indistinguishable from absent due to the bool zero value, so unattended mode forces it true regardless) lets the `pr` step's push skip the manual `AskUserQuestion` pause on a feature branch — `ship_prepare` resolves this into `pushFeatureBranchAutoApprove` (`sources` reports `"config"` or `"default"`) for the dispatched `pr` step to read. On `main`/`master`, `ship_prepare` rejects the run outright with a `DomainError` whenever `pr` is in the resolved step list — this check is unconditional and independent of `automation.push` config; a run with no `pr` step is never gated, since nothing would push.
