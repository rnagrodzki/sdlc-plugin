...
- Add `--steps`/`steps` composition not present in the user's original invocation or `.sdlc-v2/local.toml`. Pipeline composition derives from `ship_prepare`'s `steps`/`sources` output — CLI `--steps` > `--quick` > config `ship.steps[]` > built-in defaults. Legacy `--preset` and `--skip` are hard-removed; `ship_prepare` has no fields for them.
- Pass `--bump` on the CLI when `pr` is not in the resolved step list. `ship_prepare` itself rejects that combination as an error (`errors` non-empty, no state created) — a bump level has nothing to attach to without a `pr` step to forward it to. Do not work around the rejection by silently dropping `--bump` or adding `pr` back into `steps` on the user's behalf; surface the error verbatim per SKILL.md's step 4.
...
- Call `verify_tag_ancestry` from this pipeline, or otherwise reintroduce a post-version tag/ancestry check. There is no tag to check ancestry for — nothing in this pipeline creates one at ship time. Release-intent correctness is enforced synchronously by `pr_apply` itself when the pr step's Agent calls it (see SKILL.md's Decisions & gates, "Release-intent enforcement"); do not fabricate a passing check, and do not silently skip it and proceed to `pr`.
...
**Staging gap before both commit dispatches.** Neither `execute` nor `received-review` stages the files it writes, and `commit_apply` stages only tracked changes plus what is already staged — untracked paths are skipped and reported in `skippedUntrackedPaths`. ship must run `git add -A -- ':!.sdlc-v2/'` twice: between execute and commit, and again before the commit-fixes dispatch. Missing it drops the step's newly created files from the commit silently; the commit is empty only when the step created nothing but new files.
...
**Feature-branch push uses `--skip-approval` (KD-1); default-branch push is a hard gate.** The ship pipeline always passes `--skip-approval` to the dispatched `/pr` step, which skips only the Step 5 publish-confirmation prompt — it does not imply `--auto` and does not affect whether `pr_apply` runs unattended (that is controlled solely by `--auto`). On `main`/`master`, `ship_prepare` rejects the run outright with a `DomainError` whenever `pr` is in the resolved step list — this check is unconditional; a run with no `pr` step is never gated, since nothing would push.

**Config file is optional.** The pipeline runs on `ShipBuiltInDefaults` when no `ship` section exists in `.sdlc-v2/local.toml`. Do not error on a missing config — `ship_prepare` already handles this.
...
- Cases where the resolved release `level`/drafted `releaseNotes` forwarded to the pr step were wrong, or where `pr_apply` rejected them unexpectedly
