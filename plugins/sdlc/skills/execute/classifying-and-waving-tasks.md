...
changed. This LLM-side check complements the server-side duplicate-`verifyToken` warning `task-done`
returns ("verifyToken duplicates task {siblingId} — possible phantom success") — together they catch
phantom success from both ends, so run this check even when `task-done` itself reports no warning.
