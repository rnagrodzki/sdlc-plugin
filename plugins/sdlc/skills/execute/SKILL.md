...
7. **Post-wave guardrail check** — skip if `activeGuardrails` is empty. Evaluate ALL guardrails (error + warning) against the actual diff. Error FAIL → AskUserQuestion (`fix` attempts one inline fix and re-evaluates, else escalates to `override`/`cancel`; `harden` dispatches `Skill(harden)` the same way as the pre-wave gate). `--auto` → block, never override. Warning FAIL → report in the progress report, no prompt.
