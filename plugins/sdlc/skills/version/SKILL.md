...
tags. Default answer is `bumpOptions[level].suggestedPreRelease`, computed server-side from the
version config's `preReleasePolicy`: `"always-rc"` → always `"rc"`; `"continue-rc"` (the default:
once a version has an RC out, stay in RC mode until told otherwise) → `"rc"` only when
`existingRCs[bumpOptions[level].result]` is present, else empty; `"never"` → always empty. Under
`--auto`, take this default without asking. Otherwise ask whether this is another release
candidate or the final release, showing the default as the recommended choice. Choose RC when
`--rc` was passed, the (possibly defaulted) answer says so, or the user asks for one — set
`preRelease: "rc"` and show `bumpOptions[level].rcNext` as the preview version. `pr_apply`
recomputes the exact version independently when the PR is opened, so the final tag may differ if
the branch moved since.
