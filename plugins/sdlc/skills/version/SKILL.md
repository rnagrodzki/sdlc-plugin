---
name: version
description: "Use this skill when bumping a project version, creating a git release tag, generating a changelog, or performing a full semantic release workflow, updating an existing changelog entry for the current version, or retagging the current version at HEAD. Consumes pre-computed context from the `version_prepare` MCP tool and handles the complete release process. Use --changelog without a bump type to update the changelog for the already-tagged current version. Use --retag to move an existing tag to HEAD. Arguments: [major|minor|patch|<label>] [--init] [--pre <label>] [--no-push] [--changelog] [--hotfix] [--retag] [--auto]. The positional `<label>` form (e.g. `version rc`) is sugar for `--bump patch --pre <label>` and accepts any pre-release label matching `^[a-z][a-z0-9]*$`. Triggers on: version bump, create release, bump version, tag release, generate changelog, semantic versioning, semver bump, pre-release, release candidate, retag release. Use --auto to skip interactive approval prompts (release plan is still displayed)."
user-invocable: true
...
**Announce at start:** "I'm using version (sdlc v{sdlc_version})." — extract the version from the `sdlc:` line in the session-start system-reminder. If no version is in context, omit the parenthetical.
...
- There is no branch-guard field on `version_prepare`'s output at all (unlike commit's
...
1. Announce: "This skill requires write operations (editing the version file and committing). Exit plan mode first, then re-invoke `/version`."
...
- `/jira` — update Jira ticket status

## See Also

- [`/commit`](../commit/SKILL.md) — commit changes before tagging a release
- [`/jira`](../jira/SKILL.md) — update Jira ticket status after release
- [`/pr`](../pr/SKILL.md) — the PR that triggered this release
