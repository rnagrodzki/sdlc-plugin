...
commit flow (e.g. "commit this using sdlc" or invoke the `commit-sdlc`
skill if installed).
...
## 6. Resume mid-run deliver-sdlc

Seed a fixture deliver-sdlc state file for the current branch, then resume:
...
Then ask Claude to run `/deliver-sdlc --resume`.

Expect: deliver-sdlc reads the fixture file, reports resuming at the `review`
phase (the fixture's recorded `phase`), and dispatches review-sdlc next — it
does not restart from the `plan` phase or re-dispatch execute-plan-sdlc.
