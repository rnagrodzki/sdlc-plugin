...
func writeDimensionFile(root string, in DimensionsRenderInstructionsIn) (DimensionsRenderInstructionsOut, error) {
	...
	if err := os.MkdirAll(dir, 0o755); err != nil {
		return DimensionsRenderInstructionsOut{}, &mcpserver.InfraError{
			...
			Suggestion: "Create .sdlc-v2/review-dimensions manually with write permission, or free disk space on the project root, then retry dimensions_render_instructions with writeDimension true.",
			Cause:      err,
		}
	}
	...
	if err := os.WriteFile(outPath, []byte(in.Content), 0o644); err != nil {
		return DimensionsRenderInstructionsOut{}, &mcpserver.InfraError{
			Msg:        fmt.Sprintf("write %s: %s", outPath, err.Error()),
			Suggestion: "Check write permission on .sdlc-v2/review-dimensions and free disk space on the project root, then retry with the same name and content.",
			Cause:      err,
		}
	}
