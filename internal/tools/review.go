...
func saveReviewComment(projectRoot, activeRoot string, in ReviewPrepareIn) (ReviewPrepareOut, error) {
	...
	if err := os.MkdirAll(dir, 0o755); err != nil {
		return ReviewPrepareOut{}, &mcpserver.InfraError{
			...
			Suggestion: "Create " + paths.DataDir + "/reviews manually with write permission, or free disk space on the project root, then retry review_prepare with saveReview true.",
			Cause:      err,
		}
	}
	...
	if err := os.WriteFile(outPath, []byte(in.Content), 0o644); err != nil {
		return ReviewPrepareOut{}, &mcpserver.InfraError{
			...
			Suggestion: "Check write permission on " + paths.DataDir + "/reviews and free disk space on the project root, then retry review_prepare with the same content.",
			Cause:      err,
		}
	}
