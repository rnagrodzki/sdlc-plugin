...
type ExecuteStateIn struct {
	...
	TasksJSON           string         `json:"tasksJson,omitempty" jsonschema_description:"wave-start: JSON array of task objects. Each entry: {id: string, name: string, description: string, contract: string (optional), acceptanceCriteria: string (optional), files: string[] (optional)}. Entries missing required fields are dropped with a warning."`
...
- log-cli: Append a CLI-captured output block to the run's log. Requires log (the captured text), source (e.g. "go test"). Optional: branch, taskId.
