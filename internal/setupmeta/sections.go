...
type Section struct {
	...
	ConfigFile      string   // ".sdlc/config.json" | ".sdlc/local.json" | "<delegated>" | path
...
func Sections() []Section {
	return []Section{
		...
		},
		{
			...
			Purpose:         "Runtime guardrails at .sdlc/config.json#execute.guardrails evaluated by /execute and /ship before and after each wave. Error-severity violations halt execution; warning-severity violations are reported but non-blocking.",
			ConfigFile:      ".sdlc/config.json",
