...
type Section struct {
	...
	ConfigFile      string   // ".sdlc-v2/config.toml" | ".sdlc-v2/local.toml" | "<delegated>" | path
...
var versionFields = []Field{
	...
	},
	{
		...
		Label:       "Enable version-file release path",
		Type:        "boolean",
		Options:     []string{"yes", "no"},
		Default:     true,
	...
	},
	{
		...
		Label:       "Enable changelog release path",
		Type:        "boolean",
		Options:     []string{"yes", "no"},
		Default:     false,
	...
	return []Section{
		{
			...
			FilesModified:   []string{".sdlc-v2/config.toml"},
			Optional:        false,
			DelegatedTo:     "",
		...
		},
		{
			ID:              "ship",
			Label:           "ship",
			Purpose:         "Developer-local pipeline preferences for /ship: which steps run by default, default version bump, draft-PR mode, auto-approve, workspace isolation, rebase policy, and review-failure threshold. Stored in .sdlc-v2/local.toml (gitignored) so each developer can tune the pipeline without affecting teammates.",
		...
		},
		{
			...
			Purpose:         "Per-user severity allowlist for /received-review auto-apply. When set, \"agree, will fix\" findings whose severity is in the list bypass the per-finding consent gate in Step 10/12 and are auto-applied with a one-line `fixed: ...` log. Stored in .sdlc-v2/local.toml under receivedReview.alwaysFixSeverities — never in project config. Default `[]` preserves the original consent-on-every-finding behavior.",
			ConfigFile:      ".sdlc-v2/local.toml",
		...
		},
		{
			...
			Purpose:         "Custom rules at .sdlc-v2/config.toml#plan.guardrails evaluated by /plan during its critique phases. Each guardrail is a natural-language constraint (e.g., \"no direct DB access from controllers\") that flags drift in plans before execution.",
			ConfigFile:      ".sdlc-v2/config.toml",
		...
		},
		{
			...
			Purpose:         "Runtime guardrails at .sdlc-v2/config.toml#execute.guardrails evaluated by /execute and /ship before and after each wave. Error-severity violations halt execution; warning-severity violations are reported but non-blocking.",
			ConfigFile:      ".sdlc-v2/config.toml",
		...
		},
		{
			ID:              "automation",
			Label:           "automation",
			Purpose:         "Per-step pipeline automation mode (supervised/unattended), the end-of-run execution report toggle, plan-drift halting thresholds, and feature-branch push auto-approval for /execute and /ship. Stored in .sdlc-v2/local.toml (gitignored) so each developer can tune unattended-run behavior without affecting teammates.",
			ConfigFile:      ".sdlc-v2/local.toml",
