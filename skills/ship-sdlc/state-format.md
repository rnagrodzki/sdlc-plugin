...
`verify-openspec`, `archive-openspec`, `verify-pipeline`, `await-remote-review`, `learnings-commit`. **Never call `start`, `begin-step`, `complete`, `complete-step`, `skip`, or `fail` with one of these five names — the call will error.** The ported `ship-sdlc/SKILL.md` sequences these steps directly in its own prose instead:
