...
This document describes the `ship` section of `.sdlc/local.json`, the persistent configuration `ship_prepare` merges against CLI flags. Settings here apply to every `ship-sdlc` invocation in the repository unless overridden by a CLI flag, and — for the automation knobs — the separate `automation` section described below.
...
Create it manually or run `/setup-sdlc` to walk through an interactive setup. There is no `--init-config` walkthrough in this port (see "No init-config walkthrough" below).
...
| `executeWaveTimeout` | `integer` (60–3600) | `1800` | Maximum seconds a single execute wave may run. Forwarded to `execute-plan-sdlc`. The `3600` ceiling is `shipmeta.MaxWaveTimeoutSeconds`. |
| `executeWaveInterval` | `integer` (≥10) | `60` | Seconds between execute wave liveness poll attempts. Forwarded to `execute-plan-sdlc`. |
| `execute.commitWaves` | `boolean` (nested under `execute`) | `false` | Forwarded to `execute-plan-sdlc` as its per-wave commit behavior. A non-boolean value here does not error — `ship_prepare` records `commitWavesInvalidType: true` in its output instead; treat that as a warning to surface, not a hard failure. |
...
The source skill's `--init-config` entry mode ran an 8-step interactive questionnaire (steps to run, bump type, draft, auto, workspace isolation, rebase strategy, review threshold) and wrote the `ship` section via a dedicated init script. This port has no equivalent tool or script for that walkthrough. `entry-modes.md`'s `--init-config` handler redirects unconditionally to `/setup-sdlc` — read that file for the exact wording. Do not reconstruct the source questionnaire inline in this skill, even partially, and do not write `.sdlc/local.json` directly from this skill's own logic.
