...
| `git add -A -- ':!.sdlc/'` fails | Show the error, stop the pipeline | No — user action needed |
...
- Automatically resolve review findings — received-review-sdlc is always interactive unless `--auto`/`automation.mode: unattended` was explicitly forwarded.
...
- Pass `isolation: "worktree"` (or any other `isolation` value) to any Agent dispatch from this skill (R-agent-isolation-script-driven). ship-sdlc never creates an Agent SDK worktree — workspace isolation for the `execute` step is a plain `git checkout -b` run by this skill's own prose before dispatch (see "Pre-execute workspace auto-detection"). Adding `isolation: "worktree"` creates a `.claude/worktrees/agent-<id>` path that conflicts with `.sdlc/` anchoring and breaks state-file resolution. (Mirrors the same constraint already documented in `execute-plan-sdlc/SKILL.md`.)
...
**Verdict detection is text-based.** Parse the conversation for a line matching `Verdict: <VERDICT>`. The review-sdlc orchestrator always emits this. If the conversation is compacted between review and verdict parsing, the verdict may be lost — treat a missing verdict as APPROVED WITH NOTES and warn the user.
...
**Version step never creates or pushes a tag.** `version_apply` bumps the version file and writes the changelog only — see `version-sdlc/SKILL.md`'s own Gotchas ("Tag and push are manual"). This port's version-step completion must pause for the user to create and push `NEW_TAG` by hand before the pipeline can verify the side effect and proceed. This is the single largest behavioral divergence from the source skill, which trusted its own `version.js` to tag and push automatically — see the main skill's "After execute — version" section.

**Config file is optional.** The pipeline runs on `ShipBuiltInDefaults` when no `ship` section exists in `.sdlc/local.json`. Do not error on a missing config — `ship_prepare` already handles this.
...
**`.sdlc/` must be gitignored.** The `.sdlc/` directory holds developer-local config (`local.json`) and ephemeral pipeline state (`execution/`). This port's `--init-config` entry mode redirects to `/setup-sdlc` rather than writing `.gitignore` itself (see `entry-modes.md`). If `.sdlc/` is not gitignored, the staging command (`git add -A -- ':!.sdlc/'`) provides a fallback exclusion, but the gitignore remains the primary defense.
...
**No Agent SDK worktrees.** ship-sdlc isolates the `execute` step with a plain `git checkout -b <branch>` run by this skill itself, then dispatches `execute-plan-sdlc` without `--branch` so its own Step 1 sees a non-default current branch and yields `continue`. There is no `EnterWorktree`/`ExitWorktree` tool use anywhere in this pipeline, and no `isolation: "worktree"` on any Agent dispatch.
...
**Sub-skill dispatch is context-isolated.** Every sub-skill step (including `execute-plan-sdlc`) is Agent-dispatched so it loads its own SKILL.md in its own context and returns only a structured result. ship-sdlc's own context receives that structured data, not the sub-skill's definition. `execute-plan-sdlc` bounds its own context impact by dispatching one Agent per wave-task/task-cluster rather than per task; its Step-9 structured result is what this pipeline consumes to continue.
...
**No `workspace` config field, no worktree mode.** Unlike the source skill, this port's `ship_prepare` reads no `ship.workspace` config field and has no worktree-isolation path — every ship-sdlc run isolates `execute` with a feature branch. If you need `execute-plan-sdlc`'s own worktree mode, invoke `/execute-plan-sdlc --workspace worktree` standalone, outside this pipeline.
