---
name: ship-sdlc
description: "Use this skill to run the full ship pipeline: execute a plan, commit, review, version, open a PR, and optionally verify CI and await an automated reviewer. Orchestrates execute-plan-sdlc, commit-sdlc, review-sdlc, received-review-sdlc, version-sdlc, pr-sdlc, and verify-pipeline-sdlc as Agent-dispatched sub-skills, and runs a handful of steps (rebase, OpenSpec validate/archive, CI-poll, remote-review-poll, learnings commit) inline. Arguments: [--auto] [--steps <csv>] [--quick] [--quality full|balanced|minimal] [--bump patch|minor|major|<label>] [--draft] [--dry-run] [--resume] [--plan <path>] [--openspec-change <name>] [--gc] [--ttl-days <N>] [--init-config]. Triggers on: ship it, ship this, run the ship pipeline, full release pipeline, execute plan then release, ship-sdlc."
user-invocable: true
...
- **review-sdlc has no `--committed`/`--staged`/`--working`/`--worktree` flags.** Dispatch it with no arguments (or `--base <branch>` when relevant) — scope comes from `.sdlc/config.json`'s `review.scope` (default `all`), per `review-sdlc/SKILL.md`'s own Port Notes. Do not pass `--committed`.
- **received-review-sdlc is dispatched at `model: opus`**, not source's `sonnet` — matching that skill's own already-ported frontmatter.
...
1. Announce: "This skill requires write operations (executing tasks, committing, opening a PR). Exit plan mode first, then re-invoke `/ship-sdlc`."
...
|---|---|---|---|---|
| execute | Agent → execute-plan-sdlc | sonnet | `--quality`, `--rebase`, `--wave-timeout`, `--wave-interval`, `<plan-file-path>` — never `--branch`, never ship's own `--auto` (different meaning there) | Only if plan-mode-blocked |
| commit | Agent → commit-sdlc | haiku | `--auto` (ship's `flags.auto`) | no |
| review | Agent → review-sdlc | sonnet | none (or `--base <branch>`) — never `--committed` | no |
| received-review | Agent → received-review-sdlc | **opus** | `[--pr <N>] [--auto]` | YES, unless `flags.auto` |
| commit-fixes | Agent → commit-sdlc | haiku | `--auto` | no |
| rebase | inline Bash | — | — | YES on conflict |
| version | Agent → version-sdlc | haiku | `[<bump>] [--pre <label>] [--auto]` | YES — manual tag-and-push pause (Port Notes) |
...
| pr | Agent → pr-sdlc | sonnet | `[--draft] [--base <branch>]` | no |
| verify-pipeline | inline poll (`verify_pipeline_await`) + conditional Agent → verify-pipeline-sdlc | sonnet | `--pr <N> --auto` (dispatch only) | YES manual-push pause after any auto-fix commit |
| await-remote-review | inline poll (`await_remote_review`) + conditional Agent → received-review-sdlc | opus | `--pr <N> [--auto]` (dispatch only) | YES, unless `flags.auto`, on `actionable` verdict |
...
Each dispatched skill is a black box from this pipeline's point of view — never call a dispatched skill's own MCP tools directly (e.g. never call `version_prepare`/`version_apply` from ship-sdlc itself; only the version-sdlc Agent calls those). The version-sdlc dispatch specifically must be instructed to report its computed `NEW_TAG` (`tags.tagPrefix + newVersion`, per `version-sdlc/SKILL.md`'s own Step 8) in its artifacts section — this is the only way this pipeline learns the tag name for the manual tag-and-push pause and the later ancestry gate.
...
execute-plan-sdlc creates and modifies files but does not stage them:
...
`TodoWrite(todos)`. Dispatch commit-sdlc with `--auto` set to `flags.auto`.
...
`TodoWrite(todos)`. Dispatch review-sdlc with no arguments (or `--base <branch>`) — never `--committed` (Port Notes). Parse the verdict from the sub-skill's result text (`Verdict: <VERDICT>`, per `reference.md`'s "Verdict detection is text-based" Gotcha — treat a missing verdict as APPROVED WITH NOTES and warn).
...
`TodoWrite(todos)`. Dispatch received-review-sdlc **at `model: opus`** (Port Notes), forwarding `--pr <N>` if a PR already exists from a prior partial run, and `--auto` when `flags.auto`. Under `--auto`, both the consent prompt and the reply/resolve prompt are skipped, "will fix" items are auto-implemented and their threads auto-resolved, and "disagree" items are replied to but left open. Without `--auto`, the pipeline pauses here for human approval (`reference.md`'s "received-review-sdlc supports `--auto`" Gotcha).
...
`TodoWrite(todos)`. Dispatch commit-sdlc again with `--auto` set to `flags.auto`. This is a **separate** commit from the feature commit — do not squash them (`reference.md`'s "Double commit is intentional" Gotcha).
...
`TodoWrite(todos)`. Dispatch version-sdlc per the Step 2 table, instructing it (per the Dispatch protocol above) to report `NEW_TAG` in its artifacts. version-sdlc's own pipeline lands the release commit via `commit_apply` internally — its result is the only signal this pipeline needs.

**Manual tag-and-push pause (intentional exception to "never `AskUserQuestion` in `--auto`").** After version-sdlc's dispatch returns, use `AskUserQuestion` (even under `flags.auto`/`automation.mode: unattended`) to tell the user:
...
`TodoWrite(todos)`. Dispatch pr-sdlc per the Step 2 table (`--draft` from `flags.draft`).
...
- `status:"done"` — branch on `ext.verdict`:
  ...
  - `"failed"` — dispatch verify-pipeline-sdlc (model sonnet) with `--pr PR_NUMBER --logs "<ext.checks_raw>" --auto` (forwarding the raw check-name/state text; there is no full CI log fetch in this port — see `verify-pipeline-sdlc/SKILL.md`'s own Gotchas on `checks_raw` coarseness). Read its single JSON verdict line:
    - `fix-applied` — dispatch commit-sdlc `--auto` to commit the fix. **Manual-push pause**: since no tool in this port pushes, use `AskUserQuestion` to tell the user to `git push` the fix commit now, then re-poll with `verify_pipeline_await({pr: PR_NUMBER})` (fresh call, no `state_file` — a new CI run needs a new poll window). Repeat up to `flags.verifyPipelineMaxIterations` times; on exhaustion, stop and report.
...
- `"actionable"` — dispatch received-review-sdlc **at `model: opus`**, `--pr PR_NUMBER`, `--auto` set to `flags.auto`. When the fix lands via commit-sdlc, the same manual-push pause applies as in verify-pipeline above (`AskUserQuestion`, then push, before re-polling or proceeding). Record `ship_state({action:"decide", step:"await-remote-review", detail:{text:"actionable: fixed via received-review-sdlc"}})` once the fix is committed and pushed.

If `await-remote-review` is not in `flags.steps`, skip this section entirely.

### learnings-commit (inline)

Runs the ship-level Learning Capture (see [`reference.md`](reference.md)'s Learning Capture section for the exact prompts/format), appending to `.sdlc/learnings/log.md`, then commits it directly — deliberately not via a commit-sdlc Agent dispatch, since this is a single-file, non-narrative append with a fixed message:

```
commit_apply({message: "chore(ship-sdlc): capture pipeline learnings", skipConfigCheck: false, sessionID: ""}) → { sha }
```
...
If `learnings-commit` is not in `flags.steps`: `ship_state({action:"decide", step:"learnings-commit", detail:{text:"skipped: not in configured steps"}})` and do nothing (`execute-plan-sdlc`'s own Learning Capture, recorded in the feature commit, still applies regardless of this step's status).
...
- [`/execute-plan-sdlc`](../execute-plan-sdlc/SKILL.md) — dispatched by the `execute` step
- [`/commit-sdlc`](../commit-sdlc/SKILL.md) — dispatched by `commit`, `commit-fixes`, and the verify-pipeline auto-fix path
- [`/review-sdlc`](../review-sdlc/SKILL.md) — dispatched by the `review` step
- [`/received-review-sdlc`](../received-review-sdlc/SKILL.md) — dispatched by `received-review` and `await-remote-review`'s actionable path
- [`/version-sdlc`](../version-sdlc/SKILL.md) — dispatched by the `version` step
- [`/pr-sdlc`](../pr-sdlc/SKILL.md) — dispatched by the `pr` step
- [`/verify-pipeline-sdlc`](../verify-pipeline-sdlc/SKILL.md) — dispatched by the `verify-pipeline` step's failed-CI path
