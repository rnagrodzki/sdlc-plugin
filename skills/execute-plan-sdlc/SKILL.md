...
**5d-bis — OpenSpec task flip (implements R37, R39, I13, E14 — Fixes #414).** After `task-done` state writes for this wave, before the `wave-done` state write, flip OpenSpec checkboxes for refs whose plan-task siblings have all reached DONE / DONE_WITH_CONCERNS. This step runs in execute-plan-sdlc main context ONLY — never from inside any dispatched per-task/batch Agent (cite R37). When `refToTaskIds` is empty (plan has no `openspec-task` blocks), skip this step entirely (zero new behavior).
...
2. For each `(ref, siblings)` in `refToTaskIds`:
   ...
   - Interpret the outcome:
     ...
     - `not-found` or `io-error` — append to `.sdlc/learnings/log.md` (one line: `## <YYYY-MM-DD> — execute-plan-sdlc markTaskDone failed: change=<change> ref=<ref> reason=<reason>`) and add `{ change, ref, reason }` to an in-memory `openspecSyncWarnings` array surfaced by Step 9 REPORT. Pipeline continues — this is non-blocking per R39/E14.
...
```
ERROR: execute-plan-sdlc completed all waves but planned tasks are unaccounted: <missingIds>
```
...
- Touch `ship-*` state files or invoke the `ship_state` tool — ship-sdlc owns the entire ship-state lifecycle (implements R32, addresses #379). Use the `execute_state` tool for execute-state operations only.
