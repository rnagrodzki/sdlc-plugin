...
This template's content is inlined directly into `execute-plan-sdlc/SKILL.md`'s own flat-dispatch step (the wave-runner middle-agent is retired under KD15 — the main session dispatches every per-task Agent directly, the same way `review-sdlc/SKILL.md` keeps its per-dimension prompt template inline rather than in a sibling file).
