...
1. Announce: "This skill requires write operations (creating or updating a pull request). Exit plan mode first, then re-invoke `/pr-sdlc`."
...
- **OpenSpec change detection during PR creation should not block.** Unlike plan-sdlc, which
  can ask the user to disambiguate multiple active changes, pr-sdlc should silently skip
