...
This file documents Branch A ("Init") of the original `version-sdlc` skill. It is kept for
reference only.
...
`version-sdlc`'s main `SKILL.md` never dispatches here. If a project has no supported version
...
`/commit-sdlc`, and then re-run `/version-sdlc` for subsequent releases.
