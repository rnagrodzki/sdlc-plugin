...
- Tool call errors (infra failure, crash-equivalent): show the error and invoke error-report.
...
- **Skill**: setup (pr-template sub-flow)
...
- `setup --pr-template` — parent skill that invokes this sub-flow
- `setup --dimensions` — sibling sub-flow for review dimensions
- [`/pr`](../pr/SKILL.md) — uses the template written by this sub-flow
