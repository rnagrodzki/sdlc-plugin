...
> - The source's `review-sdlc/REFERENCE.md` (dimension format spec) and
>   `EXAMPLES.md` (5 sample dimensions) were never ported as standalone files —
>   `skills/review-sdlc/SKILL.md` covers the review pipeline only, not dimension
...
> (setup-sdlc) manages mode transitions. Write all dimension files in rapid
...
This sub-flow expects the parent skill (setup-sdlc) to have provided the
...
- **Tool call itself errors (infra failure):** this is a prepare-tool-class
  failure — offer to invoke error-report-sdlc, provide: Skill=setup-sdlc,
...
| `validate` tool call errors (infra failure) | MCP tool crash | Yes | Invoke error-report-sdlc with full context. |
...
- Do NOT skip the `validate({ action: "dimensions" })` step — invalid dimensions cause review-sdlc to fail silently
...
- Do NOT invoke `error-report-sdlc` for user errors or validation findings — only for `validate` tool infra failures
