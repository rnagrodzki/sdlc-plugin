...
- Do NOT accept a free-text label that isn't in `repoLabels` — the rule will be
  stripped by `/pr-sdlc`'s label evaluator later, leaving the user with a silent dead rule.
...
- [`/pr-sdlc`](../pr-sdlc/SKILL.md) — consumer; reads `pr.labels` to dispatch the label evaluator
