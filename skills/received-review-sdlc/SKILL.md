...
do not invoke `error-report-sdlc` for it.
...
this gate — even when invoked from `/ship-sdlc`, if `--auto` was not explicitly passed as a
...
- When `--auto` was **not** passed (default): for each cluster, present:
  > Cluster: file=`<file>`, findings=`<count>`. Dispatch `harden-sdlc` for this cluster?
