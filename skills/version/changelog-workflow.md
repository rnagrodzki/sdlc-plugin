...
This file documents Branch C ("Changelog-Update") of the original `version` skill. It is
...
`version`'s main `SKILL.md` never dispatches here, and a bare `--changelog` (without a
bump argument) is a no-op in this port.

## What to do instead

Edit `CHANGELOG.md` directly for the already-released version, then use `/commit` to
